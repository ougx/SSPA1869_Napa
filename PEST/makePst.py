...
class pest_pst:
    def __init__(self, pestmode='estimation', ss=False, control=None, **kw):
        if pestmode.lower().startswith('regul'):
            pestmode = 'regularisation'
        self.pestmode = pestmode.lower()

        self.par = pd.DataFrame()
        self.obs = pd.DataFrame()
        self.pargp = pd.DataFrame()
        self.tied = pd.DataFrame()
    ...
    @property
    def obsgp(self):
        if self.pestmode.startswith('regul'):
            return list(self.obs['OBGNME'].unique()) + list(self.prior['OBGNME'].unique())
        else:
            return list(self.obs['OBGNME'].unique())
    ...
    @property
    def npargp(self):
        self.pargp = self.pargp.loc[
            self.pargp.PARGPNME.str.lower().isin(self.par.PARGP.str.lower()),
            'PARGPNME INCTYP DERINC DERINCLB FORCEN DERINCMUL DERMTHD'.split()
        ]
        return len(self.pargp)

    def add_pargp(self, df):
        df = df.dropna(subset='PARGPNME INCTYP DERINC DERINCLB FORCEN DERINCMUL DERMTHD'.split())
        df.PARGPNME = df.PARGPNME.str.lower().str.strip()
        if self.ss:
        ...
        self.pargp = pd.concat([self.pargp, df]).drop_duplicates('PARGPNME')


    def add_par(self, df, ):
        df = df.dropna(subset='PARNME PARTRANS PARCHGLIM PARVAL1 PARLBND PARUBND PARGP SCALE OFFSET DERCOM'.split())
        df.PARNME = df.PARNME.str.lower().str.strip()
        df.PARGP  = df.PARGP.str.lower().str.strip()
        df.PARTRANS  = df.PARTRANS.str.lower().str.strip()
        if self.ss:
        ...
        if self.pestmode.startswith('regul'):
            df = df.dropna(subset='PRIOR WEIGHT'.split())
            df = df[(df.PARTRANS=='log')|(df.PARTRANS=='none')]
            islog = df.PARTRANS=='log'
            df['PRIOR1']  = pd.to_numeric(df.PRIOR, errors='coerce')
            isnum = ~pd.isna(df['PRIOR1'])

            df['PINME']  = df.PARNME
            df['EQ'] = ''
            df.loc[islog&isnum,    'EQ']  = [f' 1.0 * log({r.PARNME}) = {np.log10(r.PRIOR1)} ' for i,r in df.loc[islog&isnum].iterrows()]
            df.loc[islog&(~isnum), 'EQ']  = [f' 1.0 * log({r.PARNME}) - 1.0 * log({r.PRIOR}) = 0' for i,r in df.loc[islog&(~isnum)].iterrows()]
            df.loc[(~islog)&isnum,    'EQ']  = [f' 1.0 * {r.PARNME} = {r.PRIOR1} ' for i,r in df.loc[(~islog)&isnum].iterrows()]
            df.loc[(~islog)&(~isnum), 'EQ']  = [f' 1.0 * {r.PARNME} - 1.0 * {r.PRIOR} = 0' for i,r in df.loc[(~islog)&(~isnum)].iterrows()]
            df['OBGNME'] = ('regul' + df['PARGP']).str[:12]
            self.prior = pd.concat([self.prior, df.loc[df.WEIGHT>0, ['PINME', 'EQ', 'WEIGHT', 'OBGNME']]])
    ...
    def add_tied(self, df):
        ...
        tied.index = tied.index.str.lower().str.strip()
        # filtered those that are specified as tied in PAR
        tied_para = self.par.loc[self.par.PARTRANS == 'tied', 'PARNME'].str.lower().str.strip().values
        tied = tied.loc[tied_para]
        # check if any of the tieto is fixed
        fixed = tied[(self.par.set_index('PARNME').loc[tied.values, 'PARTRANS'] == 'fixed').values]
        if len(fixed) > 0:
    ...
    def add_obs(self, obs, weight_factor=1.0):
        ...
        df = obs.dropna(subset='OBSNME OBSVAL WEIGHT OBGNME'.split()).copy()
        df.OBSNME = df.OBSNME.str.lower().str.strip()
        df.OBGNME = df.OBGNME.str.lower().str.strip()
        df.WEIGHT = df.WEIGHT * weight_factor

        self.obs = pd.concat([self.obs, df['OBSNME OBSVAL WEIGHT OBGNME'.split()]])
    ...
    def get_control(self,):
        ...
        if 'win_mrun_hours' not in kw: #
            kw['win_mrun_hours'] = ""
        else:
            hr = float(kw['win_mrun_hours'])
            kw['win_mrun_hours'] = f"win_mrun_hours={hr}           "[:20]

        if 'uptestmin' not in kw:
            kw['uptestmin'] = ''
        else:
            n = int(kw['uptestmin'])
            kw['uptestmin'] = f'uptestmin={n}         '[:15]

        if 'uptestlim' not in kw:
            kw['uptestlim'] = ''
        else:
            n = int(kw['uptestlim'])
            kw['uptestlim'] = f'uptestlim={n}         '[:15]
        ...
        lines += "{0[rlambda1]:<10}{0[rlamfac]:<10}{0[phiratsuf]:<10}{0[phiredlam]:<10}{0[numlam]:<10} {0[lamforgive]} {0[derforgive]} {0[win_mrun_hours]} {0[uptestmin]} {0[uptestlim]}\n"
        ...
        lines += "{0[icov]:<10}{0[icor]:<10}{0[ieig]:<10} {0[ires]} {0[jcosave]} {0[verboserec]} {0[jcosaveitn]} {0[reisaveitn]} {0[parsaveitn]} {0[parsaverun]} {0[rrfsave]}\n"
    ...
    def get_parameter(self):
        ...
        par = self.par['PARNME PARTRANS PARCHGLIM PARVAL1 PARLBND PARUBND PARGP SCALE OFFSET DERCOM'.split()]
        lines = '* parameter data\n' + tabulate(par, showindex=False, tablefmt='plain', numalign='right', stralign='left') + '\n'

        tied = self.par['PARTRANS'] == 'tied'

        if any(tied):
            lines += tabulate(self.par.loc[tied, ['PARNME', 'TIETO']],
                              showindex=False, tablefmt='plain', stralign='left').lower() + '\n'
        return lines
    ...
    def add_prior(self, prior):
        ...
        for c in ['PINME', 'EQ', 'OBGNME']:
            prior[c] = prior[c].str.lower().str.strip()
        # check if here is fixed parameter in PI
        fixed = self.par.loc[(self.par.PARTRANS == 'fixed') | (self.par.PARTRANS == 'tied'), 'PARNME']
        fixed = '\n'.join(fixed).split()
    ...
    def add_pp(self, df):
        ...
        """
        for i, r in df.iterrows():
            if pd.isna(r.iloc[0]) or pd.isna(r.iloc[1]): continue
            vv = r.iloc[1]
            try:
                if vv == int(vv):
                    vv = int(vv)
    ...
    def write_pest_pst(self, file, comments=None):
        ...
        print(f"pst written to {file} successfully.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--add_comment',   action='append', help='')
    ...
    if args.set_ctl_xls:
        ...
        # engine: Literal["xlrd", "openpyxl", "odf", "pyxlsb"]
        control = pd.read_excel(x, s, engine=xlengine)
    if args.set_ctl_csv:
    ...
    for t in ['par', 'pargp', 'tied', 'prior', 'obs', 'io', 'pp']:
        ...
        if xlss:
            for xls in xlss:
                ...
                df = pd.read_excel(x, s, engine=xlengine)
    ...
    pst.write_pest_pst(args.pstfile, comments=args.add_comment)
